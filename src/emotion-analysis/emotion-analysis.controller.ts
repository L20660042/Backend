...
export class EmotionAnalysisController {
  ...
  async analyzeImage(
    ...
    if (!image) {
      throw new HttpException('Debe proporcionar una imagen', HttpStatus.BAD_REQUEST);
    }
    if (!userId) {
    ...
    try {
      ...
      throw new HttpException('Error al analizar: ' + error.message, HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
}
